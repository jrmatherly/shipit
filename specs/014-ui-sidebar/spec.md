...
## Additional changes (manual)

The following changes were made manually during this feature and are not part of
the core task breakdown:

- **.prettierignore** — Added `docs/poc/` so POC/static docs are excluded from
  Prettier and `pnpm format:check` passes without formatting those files.

(The `useTheme` hook in `src/presentation/web/hooks/useTheme.ts` is from feature
004-web-ui-component-library and is used by ThemeToggle from 013; no 014 change.)
