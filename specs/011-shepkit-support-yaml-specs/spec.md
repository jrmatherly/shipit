...
- [ ] All shipit-kit skills can read/write specs in YAML format (`spec.yaml`, `research.yaml`, `plan.yaml`, `tasks.yaml`)
...
| ------------------------------------------------ | ---------- | ---------------------------------------------------------------------------------------- |
| `.claude/skills/shipit-kit:*/`                     | **High**   | All skills need updated parsing logic (YAML via Node.js instead of grep/awk on Markdown) |
| `.claude/skills/shipit-kit:new-feature/templates/` | **High**   | Templates change from Markdown to YAML format                                            |
| `.claude/skills/shipit-kit:new-feature/scripts/`   | **Medium** | `init-feature.sh` updated to scaffold YAML files and generate initial Markdown           |
| `.claude/skills/shipit-kit:implement/validation/`  | **High**   | Validation gates rewritten to parse YAML instead of Markdown headers                     |
...
**L** - Touches all shipit-kit skills, templates, validation scripts, and documentation. Many files to update but each change is conceptually straightforward (parsing logic swap from grep/awk to Node.js YAML parsing).
