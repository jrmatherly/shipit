---
name: shipit-kit:fast-loop
description: Use when the user wants rapid implementation iteration without tests, builds, or commits. Triggers include "fast loop", "fast iteration", "just code", "no tests", "iterate quickly", or when the user says they have a dev server running and want to check results manually. Part of the ShipIT autonomous SDLC platform — https://github.com/jrmatherly/shipit
metadata:
  version: '1.0.0'
...
Then offer `/shipit-kit:commit-pr` or `/commit` to wrap up.
