...
All TypeScript/JavaScript in this repository (under `src/`, `packages/`, `tests/`, and any new code) MUST work correctly on **Windows, macOS, and Linux**. This is not optional. CI runs on both `ubuntu-latest` and `windows-latest`.
