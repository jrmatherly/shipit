# ShipIT-Kit: Spec-Driven Development Toolkit

> Complete reference for the `/shipit-kit:*` skill pipeline that powers autonomous feature development in Shipit AI CLI.
...
Each skill lives under `.claude/skills/shipit-kit-<name>/`.
...
### 1. `/shipit-kit:new-feature` -- Scaffold & Specify
...
### 2. `/shipit-kit:research` -- Technical Analysis
...
### 3. `/shipit-kit:plan` -- Architecture & Task Breakdown
...
### 4. `/shipit-kit:implement` -- Autonomous Executor
...
### 5. `/shipit-kit:commit-pr` -- Ship & Watch CI
...
| **Next**         | `/shipit-kit:merged` (after PR approval)             |
...
### 6. `/shipit-kit:merged` -- Post-Merge Cleanup
